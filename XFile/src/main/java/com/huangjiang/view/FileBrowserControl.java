...

{
	...
	public void initFiles() {
		...
		List<FileInfo> list = new FileManager(mContext).fillFiles(new File(mCurrentFilePath).listFiles());
	...
	public void initFiles(File file) {
		List<FileInfo> list = new FileManager(mContext).fillFiles(file.listFiles());
		Collections.sort(list);
	...
	public void onItemClick(AdapterView<?> arg0, View view, int index, long arg3) {
		...
		FileInfo file = (FileInfo) fileAdapter.getItem(index);
		if (file == null) {
			return;
		}
		if (file.isDirectory()) {
